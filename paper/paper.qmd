...
```{r}
Real_MVP_Data <- read.csv("../NBAMVP/data/01-raw_data/NBA\\ MVPs/Real_MVP_Data.csv")
...
merged_real_mvp <- merge(
  Real_MVP_Data, 
  nba_master[, c("Player", "Year", "MVP_index")],  # Select only relevant columns from nba_master
  by = c("Player", "Year"), 
  all.x = TRUE
)
...
ggplot(mvp_table, aes(x = Year, y = MVP_index)) +
  ...
  labs(
    title = "MVP Index Over the Years",
    x = "Year",
    y = "MVP Index"
  ) +
  theme_minimal() +
